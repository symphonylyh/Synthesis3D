...
public class ImageSynthesis : MonoBehaviour {
	...
	public void Save(string filename, int width = -1, int height = -1, string path = "", int specificDisplay = -1)
	{
		if (width <= 0 || height <= 0)
		{
			width = Screen.width;
		...
		StartCoroutine(
			WaitForEndOfFrameAndSave(pathWithoutExtension, filenameExtension, width, height, specificDisplay));
	}

	private IEnumerator WaitForEndOfFrameAndSave(string filenameWithoutExtension, string filenameExtension, int width, int height, int specificDisplay)
	{
		yield return new WaitForEndOfFrame();
		Save(filenameWithoutExtension, filenameExtension, width, height, specificDisplay);
	}

	private void Save(string filenameWithoutExtension, string filenameExtension, int width, int height, int specificDisplay)
	{
		if (specificDisplay == -1) {
			foreach (var pass in capturePasses)
			...
			var pass = capturePasses[specificDisplay];
		    Save(pass.camera, filenameWithoutExtension + pass.name + filenameExtension, width, height, pass.supportsAntialiasing, pass.needsRescale);
		}
	}
